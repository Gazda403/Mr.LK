...
//  Bulletproof version: works in incognito, fresh loads,
//  reduced-motion OS settings, slow networks, all browsers.
// ============================================================
...
(function () {
  ...
  if (typeof THREE === 'undefined') {
    console.error('[HeroScene] THREE.js not loaded! Check js/vendor/three.min.js');
    window.HeroScene = function () {};
    return;
  }

  // ──────────────────────────────────────────────────────────
  //  Procedural World Map Texture (Glowing Red Cities)
  // ──────────────────────────────────────────────────────────
  function buildTexture() {
    ...
    /* Ocean base: pure void black */
    ctx.fillStyle = '#000000';
    ctx.fillRect(0, 0, W, H);

    /* Subtle dark grid lines */
    ctx.strokeStyle = 'rgba(180, 35, 35, 0.12)';
    ctx.lineWidth = 1.2;
    ...
    function isLand(lon, lat) {
      if (lon > -160 && lon < -50 && lat > 15  && lat < 75)  return true;
      if (lon > -85  && lon < -35 && lat > -55 && lat < 12)  return true;
      if (lon > -10  && lon < 45  && lat > 35  && lat < 70)  return true;
      if (lon > -20  && lon < 50  && lat > -35 && lat < 37)  return true;
      if (lon > 45   && lon < 145 && lat > 5   && lat < 75)  return true;
      if (lon > 110  && lon < 155 && lat > -42 && lat < -10) return true;
      if (lon > -60  && lon < -15 && lat > 60  && lat < 84)  return true;
      return false;
    }
    ...
    cities.forEach(function (city) {
      ...
      g.addColorStop(0.35, 'rgba(180,35,35,0.7)');
      g.addColorStop(1,    'rgba(180,35,35,0)');
      ctx.fillStyle = g;
  ...
  // ──────────────────────────────────────────────────────────
  //  HeroScene Class — Bulletproof Initializer
  // ──────────────────────────────────────────────────────────
  function HeroScene() {
    this.canvas     = null;
    this.renderer   = null;
    ...
    // Boot immediately — canvas guaranteed to exist since scripts are at bottom of body
    this._safeBoot();
  }

  HeroScene.prototype._safeBoot = function () {
    var self = this;

    // Locate the canvas
    this.canvas = document.getElementById('hero-canvas');
    if (!this.canvas) {
      console.error('[HeroScene] #hero-canvas not found in DOM!');
      return;
    }

    // ── CRITICAL FIX: Force canvas to be visible BEFORE WebGL context creation ──
    ...
    // Give the browser one frame to apply styles before we init WebGL
  ...
  HeroScene.prototype._boot = function () {
    ...
    try {
      ...
      canvas.addEventListener('webglcontextlost', function (e) {
        e.preventDefault();
        console.warn('[HeroScene] WebGL context lost — will restore');
      }, false);
      canvas.addEventListener('webglcontextrestored', function () {
        console.log('[HeroScene] WebGL context restored — rebooting');
        self._boot();
      }, false);
      ...
      var rim = new THREE.DirectionalLight(0xaf2222, 1.0);
      rim.position.set(-6, 2, -2);
      this.scene.add(rim);

      // ── Globe ──
      var radius = w < 768 ? 1.2 : 1.6;
      var tex = buildTexture();  // procedural texture with void black sea & dark crimson land
      var matGlobe = new THREE.MeshPhongMaterial({
        map:       tex,
        emissive:  new THREE.Color(0x000000),  // Pitch black sea
        shininess: 5,
      });
      ...
      // ── Atmosphere Halo (Dim Crimson Rim) ──
      var matAtmo = new THREE.ShaderMaterial({
        ...
        fragmentShader: [
          ...
          '  gl_FragColor = vec4(0.7, 0.1, 0.1, clamp(i,0.0,1.0) * 0.22);',
          '}',
        ].join('\n'),
      ...
      window.addEventListener('resize', function () {
        ...
        if (self.globe) {
          var ngx = nw < 1024 ? 0 : 1.35;
          var ngy = nw < 1024 ? -0.4 : 0;
      ...
      // Start render loop
      this._tick();
      console.log('[HeroScene] ✓ Globe initialized and rendering');

    } catch (err) {
      console.error('[HeroScene] Boot failed:', err);
      // Show a CSS fallback background if WebGL completely fails
      if (this.canvas) {
        this.canvas.style.background =
          'radial-gradient(ellipse at 70% 50%, rgba(207,47,47,0.25) 0%, transparent 70%)';
      }
    }
  };
